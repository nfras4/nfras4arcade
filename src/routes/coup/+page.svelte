<script lang="ts">
  ...
  async function createRoom() {
    joining = true;
    try {
      const res = await fetch('/api/create-solo?game=coup', { method: 'POST' });
      const data: { code?: string; error?: string } = await res.json();
      if (data.error || !data.code) {
        flashError(data.error || 'Failed to create room');
        joining = false;
        return;
      }
      goto(`/coup/${data.code}`);
    } catch {
      flashError('Could not connect to server');
      joining = false;
    }
  }
  ...
  <div class="content">
    <header class="hero">
      <div class="title-frame">
        ...
        <h1 class="wordmark geo-title">Coup</h1>
        <span class="diamond-accent" aria-hidden="true"></span>
      </div>
      <p class="tagline">Bluff or call. Last one with influence wins.</p>
    </header>
    ...
    <section class="rules">
      ...
      {#if showRules}
        <ol class="rules-list fade-in">
          <li><span class="step-num" aria-hidden="true">1</span><span>Each player holds <strong>2 face-down influence cards</strong>. Lose both, you're out.</span></li>
          <li><span class="step-num" aria-hidden="true">2</span><span>On your turn pick one of seven actions: <strong>Income</strong> (+1 coin), <strong>Foreign Aid</strong> (+2), <strong>Tax</strong> (+3, claims Duke), <strong>Steal</strong> (claims Captain), <strong>Assassinate</strong> (claims Assassin, costs 3), <strong>Exchange</strong> (claims Ambassador), or <strong>Coup</strong> (costs 7).</span></li>
          <li><span class="step-num" aria-hidden="true">3</span><span>Any opponent can <strong>challenge</strong> a role claim. If the actor lacks the role, they lose an influence. If they had it, the challenger loses one.</span></li>
          <li><span class="step-num" aria-hidden="true">4</span><span>Targeted actions can be <strong>blocked</strong> by claiming a counter-role. Blocks can also be challenged.</span></li>
          <li><span class="step-num" aria-hidden="true">5</span><span>If you reach <strong>10 coins</strong> you must Coup that turn.</span></li>
          <li><span class="step-num" aria-hidden="true">6</span><span>The last player with at least one face-down card wins the pot.</span></li>
        </ol>
      {/if}
    </section>
  </div>
