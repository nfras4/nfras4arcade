<script lang="ts">
  ...
  $effect(() => {
    if ($gameState?.code) {
      goto(`/connect-four/${$gameState.code}`);
    }
  });
...
<div class="home">
  <div class="content">
    <header class="hero">
      ...
      <p class="tagline">Get four in a row to win</p>
    </header>
    ...
    <section class="rules">
      ...
      {#if showRules}
        <ol class="rules-list fade-in">
          <li><span class="step-num" aria-hidden="true">1</span><span><strong>2 players</strong> take turns dropping pieces into a 7-column grid</span></li>
          <li><span class="step-num" aria-hidden="true">2</span><span>Pieces fall to the <strong>lowest available row</strong> in the chosen column</span></li>
          <li><span class="step-num" aria-hidden="true">3</span><span>First to connect <strong>4 pieces in a row</strong> (horizontal, vertical, or diagonal) wins</span></li>
          <li><span class="step-num" aria-hidden="true">4</span><span>If the board fills up with no winner, it's a <strong>draw</strong></span></li>
        </ol>
      {/if}
    </section>
  </div>
