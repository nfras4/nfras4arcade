/**
 * WebSocket client wrapper for /ws/coup.
 * Mirrors EggnoggSocket but routes to the Coup DO.
 */
...
function getWsUrl(roomCode: string): string {
  ...
  if (typeof window === 'undefined') {
    return `ws://localhost:8787/ws/coup?room=${roomCode}${guestParam}`;
  }

  const envUrl = import.meta.env.PUBLIC_WS_URL;
  if (envUrl) return `${envUrl}/ws/coup?room=${roomCode}${guestParam}`;

  if (window.location.hostname !== 'localhost') {
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    return `${protocol}//${window.location.host}/ws/coup?room=${roomCode}${guestParam}`;
  }

  return `ws://localhost:8787/ws/coup?room=${roomCode}${guestParam}`;
}

export class CoupSocket {
  ...
  connect(roomCode: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.onerror = () => {
        reject(new Error('CoupSocket connection failed'));
      };
    });
  }
