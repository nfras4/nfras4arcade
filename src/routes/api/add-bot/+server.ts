...
export const POST: RequestHandler = async ({ request, url, platform }) => {
  ...
  const rl = await peek(rateNs, `add-bot:${ip}`, RATE_LIMIT, RATE_WINDOW);
  if (!rl.ok) {
    return json({ error: 'Too many bot adds, slow down' }, { status: 429, headers: { 'Retry-After': String(rl.retryAfter) } });
  }
  ...
  await record(rateNs, `add-bot:${ip}`, RATE_WINDOW);
